...
PORT_list = [80, 8080, 8089, 23, 21, 5001, 7001 - 7010, 8888, 6666, 1080, 27017, 6379, 1433, 3306, 1352,
             1521,
             11211,
             9200, 9300, 9090, 8069, 5900, 443, 5432, 5632, 4848, 2181]

ports = "80,8080,8089,23,21,5001,7001-7010,8888,6666,1080,27017,6379,1433,3306,1352,1521,11211," \
        "9200,9300,9090,8069,5900,443,5432,5632,4848,2181"


def masScan(ip_file, ports):
    ip_file = ip_file
    json_name = str(time.time()) + ".log"
    payload = "masscan -iL {0} -p {2} -oJ {1} --rate 3000".format(ip_file, json_name, ports)
    print(payload)
    os.system(payload)
...
def callback_result(host, scan_result):
    if host in scan_result['scan'].keys() and 'tcp' in scan_result['scan'][host]:
...
def nmapScan(target_list):
    ...
    for target in target_list:
        scanner.scan(target, arguments='-sV -PS -p' + ','.join(target_list[target]), callback=callback_result)
